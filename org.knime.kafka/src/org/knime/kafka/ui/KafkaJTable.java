...
/**
 * The kafka table allowing to dynamically add and remove Kafka properties.
 *
 * @author Mark Ortmann, KNIME GmbH, Berlin, Germany
 *
 */
final class KafkaJTable extends JTable {
    /** Default uid. */
    ...
    /** The wrong intput color. */
    private static Color WRONG_INPUT_COLOR = new Color(255, 120, 120);

    /**
     * Cosntructor.
     *
     * @param dm the Kafka model
     */
    KafkaJTable(final KafkaModel dm) {
        super(dm);
    ...
    public KafkaModel getModel() {
        return (KafkaModel)super.getModel();
    }

    /**
     * Sets the {@link KafkaCellEditor} to the KEY_IDX column.
     *
     * @param model the Kafka mdoel
     */
    private void setCellEditor(final TableColumnModel model) {
        model.getColumn(KafkaModel.KEY_IDX)
            .setCellEditor(new KafkaCellEditor(getModel().getProvider(KafkaModel.KEY_IDX)));
